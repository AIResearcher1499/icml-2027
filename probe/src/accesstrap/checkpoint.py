...
from accesstrap.generate import Sample
...
def sample_key(item_id: str, condition: str, sample_idx: int) -> tuple[str, str, int]:
    return (item_id, condition, int(sample_idx))


def truncate_torn_tail(path: Path) -> None:
    """Drop a last line that is not valid JSON (crash mid-write)."""
    ...
    if text.endswith("\n"):
        ...
        # no trailing newline → last line is almost certainly torn
        pass
...
def load_samples(path: Path) -> tuple[list[Sample], set[tuple[str, str, int]]]:
    """Load complete JSONL rows. A torn last line (crash mid-write) is skipped."""
    samples: list[Sample] = []
    ...
    with path.open(encoding="utf-8") as f:
        for line_no, line in enumerate(f, start=1):
            ...
            try:
                ...
                continue
            key = sample_key(s.item_id, s.condition, s.sample_idx)
            if key in done:
                continue
            done.add(key)
            samples.append(s)
