...
from scratchdepth.dummy import Sample
...
def sample_key(item_id: str, depth: int, condition: str) -> tuple[str, int, str]:
    return (item_id, int(depth), str(condition))
...
def load_samples(path: Path) -> tuple[list[Sample], set[tuple[str, int, str]]]:
    samples: list[Sample] = []
